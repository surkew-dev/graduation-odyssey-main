...
export function MazeGame({ onComplete }: MazeGameProps) {
  ...
  return (
    <section className="page-bg" style={{ minHeight:"100dvh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16, padding:"80px 16px 24px" }}>
      ...
      {/* Maze board — sized to always fit between header and dpad */}
      <div
        onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
        style={{
          flexShrink: 0,
          position: "relative",
          width: "min(calc(100vw - 32px), calc(100dvh - 300px), 420px)",
          aspectRatio: "1 / 1",
      ...
      >
        <div style={{ position:"relative", width:"100%", height:"100%", borderRadius:12, overflow:"hidden", background:"#f9f5ec" }}>
          ...
          <div style={{ position:"absolute", pointerEvents:"none", width:`${100/COLS}%`, height:`${100/ROWS}%`, left:`${goal.c*100/COLS}%`, top:`${goal.r*100/ROWS}%`, display:"grid", placeItems:"center" }}>
            <div style={{ width:"62%", height:"62%", borderRadius:"50%", background:"var(--gold)", display:"grid", placeItems:"center", boxShadow:"0 0 14px rgb(212 146 42/0.5)", animation:"pulse-soft 2s ease-in-out infinite" }}>
              <ScrollText size={14} color="#fff" />
            </div>
          </div>
          ...
          <div style={{ position:"absolute", pointerEvents:"none", width:`${100/COLS}%`, height:`${100/ROWS}%`, left:`${pos.c*100/COLS}%`, top:`${pos.r*100/ROWS}%`, display:"grid", placeItems:"center", transition:"left 0.16s cubic-bezier(0.22,1,0.36,1), top 0.16s cubic-bezier(0.22,1,0.36,1)" }}>
            <div style={{ width:"62%", height:"62%", borderRadius:"50%", background:won?"var(--gold)":"var(--sky)", display:"grid", placeItems:"center", boxShadow:`0 0 14px ${won?"rgb(212 146 42/0.6)":"rgb(59 130 246/0.5)"}`, transform:won?"scale(1.25)":"scale(1)", transition:"all 0.3s cubic-bezier(0.34,1.56,0.64,1)" }}>
      ...
      {/* D-pad — proper 3x3 cross */}
      <div style={{ flexShrink:0, display:"grid", gridTemplateColumns:"repeat(3, 48px)", gridTemplateRows:"repeat(3, 48px)", gap:6 }}>
        <span /><DBtn onClick={()=>move("N")}><ChevronUp size={18}/></DBtn><span />
        <DBtn onClick={()=>move("W")}><ChevronLeft size={18}/></DBtn><span /><DBtn onClick={()=>move("E")}><ChevronRight size={18}/></DBtn>
        <span /><DBtn onClick={()=>move("S")}><ChevronDown size={18}/></DBtn><span />
      </div>
